 ...
 */
export const getTypeAndTotalToSVGPosition = ({ type, isLast = false }: getTypeAndIsEdgeToSVGPositionProps) => {
  switch (type) {
    case 'A':
      if (!isLast) {
        // 다음 페이지가 존재한다면
 ...
 */
export const calculateNeededElementsCount = <T>(array: T[], length: number, isFirst?: boolean) => {
